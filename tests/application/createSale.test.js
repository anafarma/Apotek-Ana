...
function deps({ stock = 100, commitSale = async tx => ({ success: true, transactionId: tx.transactionId, total: tx.total }) } = {}) {
  ...
  return {
    requestLedger: {
      async get(id) { return requests.get(id) ?? null; },
      async begin(row) { requests.set(row.requestId, { status: 'IN_PROGRESS' }); },
...
test('server resolves box price independently from strip price', async () => {
  const result = await createSale({
    requestId: 'req-box-1', shiftId: 'shift-1', actor: { userId: 'cashier-1' },
...
test('server converts box quantity to base stock quantity', async () => {
  let committed;
  const result = await createSale({
    requestId: 'req-box-2', shiftId: 'shift-1', actor: { userId: 'cashier-1' },
    items: [{ productId: 'amlodipine', unitId: 'box', qty: 2 }],
    payment: { method: 'cash', amount: 70000 }
  }, deps({ commitSale: async tx => { committed = tx; return { transactionId: tx.transactionId, total: tx.total }; } }));
...
test('retry of completed request returns original result without recommit', async () => {
  ...
  const command = {
    requestId: 'req-idempotent', shiftId: 'shift-1', actor: { userId: 'cashier-1' },
    items: [{ productId: 'amlodipine', unitId: 'strip', qty: 1 }],
...
test('sale is rejected when base stock is insufficient', async () => {
  await assert.rejects(() => createSale({
    requestId: 'req-stock', shiftId: 'shift-1', actor: { userId: 'cashier-1' },
    items: [{ productId: 'amlodipine', unitId: 'box', qty: 2 }],
    payment: { method: 'cash', amount: 70000 }
  }, deps({ stock: 10 })), /INSUFFICIENT_STOCK/);
});
