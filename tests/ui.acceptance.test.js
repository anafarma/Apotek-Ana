...
test('UI exposes product, unit and quantity controls', () => {
  assert.ok(has('productSelect')); assert.ok(has('unitSelect')); assert.ok(has('qtyInput')); assert.ok(has('addBtn'));
});

test('UI explicitly supports independent strip and box prices', () => {
  assert.match(js, /Strip.*4000/); assert.match(js, /Box \(10 Strip\).*35000/);
  assert.match(js, /total:qty\*u\.price/); assert.doesNotMatch(js, /u\.conversion\s*\*\s*u\.price/);
});
...
test('offline UI queues requests and persists them until sync acknowledgement', () => {
  assert.match(js, /state\.queue\.push\(request\)/); assert.match(js, /saveQueue\(\)/);
  assert.match(js, /stok belum dianggap terjual sampai sync berhasil/); assert.match(js, /API V2 belum dikonfigurasi; queue dipertahankan dan TIDAK dihapus/);
});
...
class FakeElement {
  ...
  set innerHTML(value){
    ...
    if((this.id==='productSelect'||this.id==='unitSelect') && !this.value){
      const match=value.match(/<option\s+value="([^"]+)"/i); if(match) this.value=match[1];
    }
  }
...
test('runtime UI selects Box and calculates the independent Box price', () => {
  const {elements}=runtimeHarness(); assert.match(elements.unitSelect.innerHTML,/Box \(10 Strip\)/);
  elements.unitSelect.value='OB0015-BOX'; elements.unitSelect.change(); assert.match(elements.unitPrice.textContent,/35\.000/);
  elements.qtyInput.value='1'; elements.addBtn.click(); assert.match(elements.cartBody.innerHTML,/Box \(10 Strip\)/); assert.match(elements.cartTotal.textContent,/35\.000/);
});
test('runtime UI rejects Box quantity that exceeds base-unit stock', () => {
  const {elements}=runtimeHarness(); elements.unitSelect.value='OB0015-BOX'; elements.unitSelect.change(); elements.qtyInput.value='5'; elements.addBtn.click();
  assert.match(elements.validation.textContent,/Stok tidak cukup/); assert.equal(elements.cartBody.innerHTML,'');
});
test('runtime UI rejects insufficient payment before API call', async () => {
  let calls=0; const {elements}=runtimeHarness({apiUrl:'https://v2.invalid/api',fetchImpl:async()=>{calls++;throw new Error('must not call API');}});
