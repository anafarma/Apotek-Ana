...
const state = { cart:[], offline:false, queue:[] };
...
function checkout(){
  ...
  if(state.offline){state.queue.push(request);renderQueue();showResult('result','Transaksi disimpan ke offline queue; stok belum dianggap terjual sampai sync berhasil.','ok');}
  else showResult('result',`Transaksi valid. Total ${rupiah(total)}; kembalian ${rupiah(paid-total)}.`,'ok');
...
function sync(){ if(state.offline){showResult('queueResult','Tidak dapat sync saat offline.','error');return;} if(!state.queue.length){renderQueue();return;} const n=state.queue.length;state.queue=[];renderQueue();showResult('queueResult',`${n} request siap dikirim ke RequestLedger/API dan harus idempotent.`,'ok'); }
...
renderProducts();renderCart();renderQueue();$('trustBadge').textContent='MASTER: TRUSTED (HARNESS)';
