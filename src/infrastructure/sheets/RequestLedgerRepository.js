...
export class SheetsRequestLedgerRepository {
  ...
  get(requestId) {
    const x=this._row(requestId); if(!x) return null;
    return Object.fromEntries(x.headers.map((h,i)=>[h,x.values[i]]));
  }
  claim({requestId,payloadHash,action,createdAt}) {
    ...
    if(existing) {
      ...
      if(String(record.PayloadHash)!==String(payloadHash)) throw new Error('Idempotency conflict: payload hash differs');
  ...
  complete(requestId, transactionId, resultJson, completedAt) {
    this._set(requestId,{Status:'COMPLETED',TransactionId:transactionId,ResultJson:JSON.stringify(resultJson??{}),CompletedAt:completedAt,ErrorCode:''});
  }
